  ...
  return (
    <div>
      <PageHeader title="Notice" subtitle="Publish notices to students, teachers and parents"
        action={<button onClick={() => { setForm({ title:'',content:'',target:'All',date:'' }); setModal(true) }} className="btn-primary"><Plus className="w-4 h-4" /> Add Notice</button>}
      />
      <div className="card">
      ...
      <Modal open={modalOpen} onClose={()=>setModal(false)} title="Add Notice"
